<?php
    ...
    //이 배열변수에서 원하는 값들을 추출
    $user_id= $_POST['user_id'];  // html의 name 속성에 있는 이름.
    ...
    $message= nl2br($message); // new line을 br로 바꿔주는 함수..

    // 실무에서는 이 데이터들을 Database에 저장하는 등의 코드를 작성함. 
    // 내일 할것이다. 
    // 오늘은 이 예제에서는 간단하게 데이터를 잘 받았는지 확인하는 정도로...
    // 받은 데이터들을 사용자에게 그대로 응답response해주기(브라우저에 보여주기) 
    // 그래서 request와 response의 규약이라고 함.
    ...
    // checkbox는 다중선택으로 여러개의 값을 동시에 보내기에 배열로 받아짐. 
    $fruits=$_POST['fruits'];
    //$fruits는 배열변수임 - 파이썬의 리스트와 같은 구조.. 인덱스 번호 사용한다는 의미
